...
"#
    ...
    let mut judge_code = format!("cargo b -r -q --bin {solution}\n");
    for test in &tests {
        judge_code += &format!("\"$@\" target/release/{solution} {dir} {problem} {test}\n");
    }
    std::fs::create_dir_all("src/bin").unwrap();
    std::fs::create_dir_all("tests").unwrap();
    std::fs::create_dir_all("benches").unwrap();
    std::fs::write(&main_path, main_code).expect("Failed to write main file");
    ...
    std::fs::write(&judge_path, judge_code).expect("Failed to write judge file");
}
...
pub fn remove_test(solution: &str) {
    ...
    let bench_path = format!("benches/{solution}.rs");
    let judge_path = format!("../judge_{solution}.sh");
    std::fs::remove_file(&main_path).expect("Failed to remove main file");
    ...
    std::fs::remove_file(&judge_path).expect("Failed to remove judge file");
}
...
pub fn main(testfn: impl Fn(Reader, &mut dyn Write) -> Result) {
    ...
    #[cfg(debug_assertions)]
    {
        ...
        let rd = unsafe { Reader::new(input_data.as_ptr()) };
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        let problem = std::env::args().nth(2).unwrap();
        let test = std::env::args().nth(3).unwrap();
        ...
        std::println!("test {test:<40} ... bench: {duration:>10} ns/iter");
    }
}

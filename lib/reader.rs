...
impl Reader {
    ...
    #[inline]
    const fn parse_from_u128(x: u128) -> u64 {
        let mut l = x as u64;
    ...
    #[inline]
    const fn parse_remaining_u32(&mut self, mut num: u32) -> u32 {
    ...
    #[inline]
    const fn parse_remaining_u64(&mut self, mut num: u64) -> u64 {
        let mut cur = self.pop();
        while cur < 10 {
    ...
    /// # Safety
    ///
    /// The caller must guarantee that a readable region of memory starting at
    /// `ptr` exists when it is accessed.
    #[inline]
    ...
    #[inline]
    pub const fn digit(&mut self) -> u8 {
        let num = self.pop();
        self.add(1);
        num
    }
    ...
    /// Reads a `u32` always with 8 digits or fewer.
    #[inline]
    pub const fn u26(&mut self) -> u32 {
    ...
    /// Reads a `u64` always with 16 digits or fewer.
    #[inline]
    pub const fn u53(&mut self) -> u64 {
    ...
    /// Reads an `i32` always with 8 digits or fewer.
    #[inline]
    pub const fn i26(&mut self) -> i32 {
        if self.neg() {
    ...
    /// Reads an `i64` always with 16 digits or fewer.
    #[inline]
    pub const fn i53(&mut self) -> i64 {
        if self.neg() {
